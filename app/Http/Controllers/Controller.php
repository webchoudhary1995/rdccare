...
class Controller extends BaseController
{
    ...
    public function getUserlist($roleId=3){
        $user = User::where('user_type',$roleId)->latest()->get();
        
        return $user;
    }
    public function getpkglist($roleId=3){
        $Package = Package::get();
        foreach($Package as &$pkg){
            $mrp = 0;
            // $pkg->price = $pkg->mrp;
            $find_pa = TestDetails::where("package_id", $pkg->id)->get();
            $parameter = 0;
            $p_data = array();
            $sm_typeid = explode(',',$pkg->sample_type);
            $sm_type = array();
            foreach($sm_typeid as $sm){
                $sm_type[]=SampleType::find($sm) ? SampleType::find($sm)->sample_name : '';
            }
            $pkg->sample_type = implode(',',$sm_type);
            foreach ($find_pa as $d) {
                ...
                if ($d->type == 2) {
                    ...
                    $nm= Profiles::find($d->type_id) ? Profiles::find($d->type_id)->profile_name : '';
                    $parameter2 = 0;
                    if ($a) {
                        $arr = explode(",", $a->no_of_parameter);
                        $parameter = $parameter + count($arr);
                        $parameter2 = $parameter2 + count($arr);
                            foreach ($arr as $l) 
                            {
            ...
            $pkg->no_of_parameter = $parameter;
            $pkg->paramater_data = $p_data;
            $pkg->price = $mrp;
            // dd($pkg->paramater_data);
        }
        // dd($Package);
        return $Package;
    }
    public function gettestlist($roleId=3){
        $Profiles = Profiles::get();
        foreach($Profiles as $Profile){
            $Profile->price = $Profile->mrp;
            $parameter = 0;
            $p_data = array();
            $sm_typeid = explode(',',$Profile->sample_type);
            $sm_type = array();
            foreach($sm_typeid as $sm){
                $sm_type[]=SampleType::find($sm) ? SampleType::find($sm)->sample_name : '';
            }
            $Profile->sample_type = implode(',',$sm_type);
            $arr = explode(",", $Profile->no_of_parameter);
            $parameter = $parameter + count($arr);
            foreach($arr as $l){
                    $p_data[]= Parameter::find($l) ? Parameter::find($l)->name : '';
                }
            $Profile->no_of_parameter = $parameter;
            $Profile->paramater_data = $p_data;
        }
        return $Profiles;
    }
    public function getparmslist($roleId=3){
        $Parameter = Parameter::get();
        foreach($Parameter as $Parameters){
            $Parameters->price = $Parameters->mrp;
            $sm_typeid = explode(',',$Parameters->sample_type);
            $sm_type = array();
            foreach($sm_typeid as $sm){
                $sm_type[]=SampleType::find($sm) ? SampleType::find($sm)->sample_name : '';
            }
            $Parameters->sample_type = implode(',',$sm_type);
        }
        return $Parameter;
    }
